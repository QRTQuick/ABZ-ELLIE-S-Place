    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-8 text-sm font-medium text-slate-600 uppercase tracking-widest">
          ...
          <Link to="/current-stock" className="hover:text-yellow-600 transition-colors">Current Stock</Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white absolute top-full left-0 w-full shadow-xl border-t border-slate-100 p-6 flex flex-col space-y-4 animate-in slide-in-from-top duration-300">
          <Link to="/" className="text-lg font-medium text-slate-800" onClick={() => setIsMobileMenuOpen(false)}>Home</Link>
          <Link to="/shop" className="text-lg font-medium text-slate-800" onClick={() => setIsMobileMenuOpen(false)}>Shop</Link>
          <Link to="/current-stock" className="text-lg font-medium text-slate-800" onClick={() => setIsMobileMenuOpen(false)}>Current Stock</Link>
          <Link to="/about" className="text-lg font-medium text-slate-800" onClick={() => setIsMobileMenuOpen(false)}>About</Link>
          <Link to="/contact" className="text-lg font-medium text-slate-800" onClick={() => setIsMobileMenuOpen(false)}>Contact</Link>
        </div>
      )}
    </nav>
  );
